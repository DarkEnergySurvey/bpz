...
class p_c_z_t:
    def __init__(self, f, ef, ft_z):
        ...
        # sum(flux * Flux_temp / err^2) for each (z,template)
        #print 'np.shape(ft_z)', np.shape(ft_z)
        self.fot = add.reduce(
            where(nonobs, 0., reshape(f, (1, 1, self.nf)) * ft_z / reshape(ef, (1, 1, self.nf)) ** 2)
            , -1)

        # ftt = SUM(Flux_temp / err)^2 for each (redshift, template)
        self.ftt = add.reduce(
        ...
        #print 'self.chi2', np.shape(self.chi2)
        #1 / 0
