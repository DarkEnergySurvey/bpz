...
gr_targ = target['OMAG'][:,0]-target['OMAG'][:,1]
ri_targ = target['OMAG'][:,1]-target['OMAG'][:,2]
iz_targ = target['OMAG'][:,2]-target['OMAG'][:,3]

targ_data = np.vstack((target['OMAG'][:,2],gr_targ,ri_targ,iz_targ))
...
c1 = pyfits.Column(name='ID', format='K', array=spec['ID'][mask])
